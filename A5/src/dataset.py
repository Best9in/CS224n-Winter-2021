...
class CharCorruptionDataset(Dataset):
    #A class that yields examples of a simplified span corruption objective. Do not change the signature of the __init__ or __getitem__ functions.
    ...
    def __getitem__(self, idx):
        ...
        if truncate_len >= document_len:
            ...
            truncate_document = document[:truncate_len] #切片
        ...
        # Uniform:
        mean_masked_content_len = len(truncated_document) / 4 
        noise = random.uniform(-1/8,1/8) * len(truncated_document) #为什么要在|1/8|内???
        masked_content_len = mean_masked_content_len + noise #加点噪音???
        # Gaussian:
        # masked_content_len = random.gauss(1/4, 0.05) * len(truncated_document)
        masked_content_len = int(np.clip(masked_content_len, 1, truncate_len - 2)) #转化为整数，一共break成三个部分，最大值为truncate_len -2
        
        #开始break
        mask_idx = random.randint(1, truncate_len - masked_content_len -1)   #最小值是1，留一个给prefix
        prefix = truncated_document[:mask_idx]
        masked_content = truncated_document[mask_idx : mask_idx + masked_content_len]
        suffix = truncated_document[mask_idx + masked_content_len:]
        ...
        masked_string + = self.PAD_CHAR*(self.block_size - len(masked_string))  #truncate_len + masks,= maksed_string,(+) PAD = block_size
